...
class BartForConditionalGenerationOutput(Seq2SeqLMOutput):
    ...
    last_hidden_state_nk: Optional[torch.Tensor] = None
    knowledge_relevance: Optional[torch.Tensor] = None
    loss_nk: Optional[torch.Tensor] = None
...
class BartModel(BaseBartModel):
    ...
    def forward(
        ...
        return BartModelOutput(
            ...
            last_hidden_state_nk=decoder_outputs.last_hidden_state,
...
class BartWithKnowledgeForConditionalGeneration(BartForConditionalGeneration):
    ...
    def forward(
        ...
        # Without knowledge
        lm_logits_nk = self.lm_head(outputs.last_hidden_state_nk) + self.final_logits_bias
        ...
        return BartForConditionalGenerationOutput(
            loss=masked_lm_loss,
            loss_nk=masked_lm_loss_nk,
            logits=lm_logits,
            last_hidden_state=outputs.last_hidden_state,
            last_hidden_state_nk=outputs.last_hidden_state_nk,
            past_key_values=outputs.past_key_values,
            decoder_hidden_states=outputs.decoder_hidden_states,
            decoder_attentions=outputs.decoder_attentions,
            cross_attentions=outputs.cross_attentions,
