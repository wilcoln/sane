...
def prune_conceptnet_df(node_ids_list):
    ...
    return conceptnet_df[conceptnet_df['source'].isin(concept_df['name'].loc[node_ids]) & conceptnet_df['target'].isin(
        concept_df['name'].loc[node_ids])]
...
def concept_ids_to_pyg_data(conceptnet_df, node_ids_list):
    ...
    for node_ids in tqdm(node_ids_list):
        ...
        triples_df = conceptnet_df[
            conceptnet_df['source'].isin(concept_df['name'].loc[node_ids]) & conceptnet_df['target'].isin(
                concept_df['name'].loc[node_ids])]
        # Load nodes
