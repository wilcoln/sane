...
class Explainer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # send tensors to gpu
        encoded_inputs = {k: v.to(settings.device) for k, v in inputs['encoded_sentences'].items()}
        encoded_labels = {k: v.to(settings.device) for k, v in inputs['encoded_explanations'].items()}
