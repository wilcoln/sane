...
class Encoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        edge_index, edge_attr = subgraph(subset, conceptnet.pyg.edge_index, conceptnet.pyg.edge_attr, relabel_nodes=True)
...
if __name__ == '__main__':
    ...
    inputs = {'Sentences_embedding': torch.randn(2, settings.sent_dim),
              'concept_ids': [torch.tensor([0, 1, 2, 3, 10 , 11, 1005]), torch.tensor([0, 1, 3, 4, 5, 6, 1001, 1002, 1003])]}
    out = encoder(inputs)
    ic(out)
