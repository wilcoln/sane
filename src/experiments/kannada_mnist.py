...
class Trainer(TorchModuleBaseTrainer):
    def __init__(self, model, optimizer, train_loader, val_loader, loss_fn, params):
...
if __name__ == '__main__':
    ...
    _slice = {
        ...
        'tsne': slice(2, 4),
        'umap': slice(4, 6),
        'tsne+umap': slice(2, None),
    }
    results = {}
    keys = None
    knowledge_list = ['none', 'random', 'tsne', 'umap', 'tsne+umap']
    for knowledge in tqdm(knowledge_list):
        ...
        for _ in range(params['num_runs']):
            ...
            if knowledge == 'none':
                model = SimpleSANENoKnowledge(x_dim, k_dim, y_dim, h_dim).to(settings.device)
            else:
            ...
            # Create the trainer
            trainer = Trainer(model, optimizer, train_loader, val_loader, loss_fn, params)
    ...
    for split in {'train', 'val'}:
        for metric in metrics:
            for knowledge in knowledge_list:
                ...
                plt.plot(range(params['num_epochs']), means, label=capitalize(knowledge))
                plt.fill_between(range(params['num_epochs']), means - stds, means + stds, alpha=0.2)
