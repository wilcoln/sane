...
class BartModel(BartPretrainedModel):
    def __init__(self, config: BartConfig, z=True):
        super().__init__(config)
...
[DOCS]
    ...
    def forward(
        ...
        if z:
            ...
            z_mask = enocoder_attention_mask.unsqueeze(-1).float() * z.squeeze(1)  # [B, T, 1]
            masked_enc_outputs = encoder_outputs[0] * z_mask
        ...
        decoder_outputs = self.decoder(
            ...
            encoder_hidden_states=masked_enc_outputs,
        ...
        # L1 on z
        if z:
            weights = torch.cat([x.view(-1) for x in self.z.parameters()])
