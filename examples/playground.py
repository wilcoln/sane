...
# Load test set
dataloader = DataLoader(ESNLIDataset(path=settings.data_dir, split='test', frac=.25, chunk=0), batch_size=settings.batch_size, shuffle=False, num_workers=settings.num_workers, collate_fn=collate_fn)

# Load model
model = KAXWK(metadata=conceptnet.metadata()).to(settings.device)
model_path = osp.join(settings.results_dir, 'trainers/2022-08-03_20-33-55_636021_dataset=esnli_train_0_model=KAXWK_num_epochs=5/model.pt')
